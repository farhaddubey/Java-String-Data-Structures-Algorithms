...
public class KMP{
    ...
    private int[] buildLPS(String pattern){
        ...
        int[] lps=new int[m]; 
        int length=0; // As at 0th index length of LPS is always 0
        ...
        while(i<m){
            if(pattern.charAt(i)==pattern.charAt(length)){
                ...
                if(length!=0){
                    length=lps[length-1]; // falling back to the previous longest prefix suffix 
                }else{
                    lps[i]=0;
                    i++;
                }
    ...
    public int KMPSearch(String text, String pattern){
        int n=text.length(); 
        int m=pattern.length(); 
        // Edge case: if the pattern is empty, then returning 
        if(m==0) return 0; 
